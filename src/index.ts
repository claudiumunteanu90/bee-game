...
const addDroneButton = document.getElementById(
  "add-drone-button"
) as HTMLButtonElement;
const playerNameSelect = document.getElementById(
  "player-name"
...
const updateSwarmInfo = () => {
  queenDetails!.innerText = `${BeeType.Queen}: ${
    bees.find((bee) => bee.type === BeeType.Queen)?.hp || 0
  } HP`;

  const workerBees = bees.filter((bee) => bee.type === BeeType.Worker);
  const workerNumber = workerBees.length;
  const workerTotalHealth = workerBees.reduce(
    (total, bee) => total + bee.hp,
    0
  );
